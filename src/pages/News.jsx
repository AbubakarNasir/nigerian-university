...
const FILTER_TABS = ["All", "News", "Events", "Announcements"];

// Unsplash — free to use, no attribution required (Unsplash License)
const POSTS = [
  {
    title: "Northbridge University Wins National Academic Excellence Award",
    date: "May 15, 2025",
    category: "News",
    image: "https://images.unsplash.com/photo-1760348082270-3a46a3512850?auto=format&fit=crop&w=300&h=300&q=70",
  },
  {
  ...
  {
    title: "New STEM Center Officially Opens",
    date: "April 10, 2025",
    category: "News",
    ...
    <div className="nu-page">
      ...
      <section className="nu-page-header">
        ...
        <div className="nu-page-header__inner">
          <h1 className="nu-anim nu-anim--1">News & Events</h1>
          <p className="nu-anim nu-anim--2">Stay updated with the latest from our university</p>
        </div>
      </section>

      {/* ---------------------------------- Feed ---------------------------------- */}
      <section className="nu-feed">
        <div className="nu-container">
          <div className="nu-tabs" role="tablist" aria-label="Filter news and events">
            {FILTER_TABS.map((tab) => (
              <button
                key={tab}
                role="tab"
          ...
          <div className="nu-post-list">
            {visiblePosts.length === 0 && (
              <p className="nu-post-list__empty">Nothing here yet — check back soon or try a different filter.</p>
            )}
            {visiblePosts.map((post) => (
              <Link to="/news-events" className="nu-post-row" key={post.title}>
                <div className="nu-post-row__image">
                  <img src={post.image} alt={post.title} />
                </div>
                <div className="nu-post-row__body">
                  <h3>{post.title}</h3>
                  <div className="nu-post-row__meta">
                    <span className="nu-post-row__date">{post.date}</span>
                    <span className="nu-badge">{post.category}</span>
                  </div>
                </div>
                <IconArrowRight className="nu-post-row__arrow" width="18" height="18" />
              </Link>
            ))}
          </div>
        </div>
