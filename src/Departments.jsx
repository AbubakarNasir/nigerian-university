...
const FILTER_TABS = ["All", "Engineering", "Business", "Sciences", "Arts & Humanities", "Education"];
const FACULTIES = ["All Faculties", "Engineering", "Business", "Sciences", "Arts & Humanities", "Education"];

// Unsplash — free to use, no attribution required (Unsplash License)
const DEPARTMENTS = [
  {
    name: "Department of Computer Engineering",
    description: "Advancing computing hardware, embedded systems and electronics research and education.",
    faculty: "Engineering",
    image: "https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=300&h=300&q=70",
  },
  {
    name: "Department of Computer Science",
    description: "Exploring software engineering, algorithms, AI and data science through hands-on learning.",
    faculty: "Engineering",
    image: "https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&w=300&h=300&q=70",
  },
  {
    name: "Department of Business Administration",
    description: "Preparing leaders in management, finance, marketing and entrepreneurship for a global career.",
    faculty: "Business",
  ...
  },
  {
    ...
    description: "Training future physicians through rigorous clinical practice and academic study.",
    faculty: "Sciences",
  ...
  },
  {
    ...
    description: "Fostering critical thinking through literature, history, philosophy and the arts.",
    faculty: "Arts & Humanities",
    image: "https://images.unsplash.com/photo-1741699427799-3fbb70fce948?auto=format&fit=crop&w=300&h=300&q=70",
  },
  {
    name: "Department of Education",
    description: "Shaping the next generation of educators and academic leaders.",
    faculty: "Education",
    image: "https://images.unsplash.com/photo-1758270704286-83476deb3bd1?auto=format&fit=crop&w=300&h=300&q=70",
  },
];
...
export default function Departments() {
  ...
  const visibleDepartments = DEPARTMENTS.filter((d) => {
    const matchesTab = activeTab === "All" || d.faculty === activeTab;
    const matchesFaculty = faculty === "All Faculties" || d.faculty === faculty;
    const matchesQuery = d.name.toLowerCase().includes(query.trim().toLowerCase());
    return matchesTab && matchesFaculty && matchesQuery;
  });
    ...
    <div className="nu-page">
      ...
      <section className="nu-page-header">
        ...
        <div className="nu-page-header__inner">
          <h1 className="nu-anim nu-anim--1">Departments</h1>
          <p className="nu-anim nu-anim--2">Explore our academic departments and faculties</p>
        </div>
      </section>
        ...
        <div className="nu-container">
          ...
          <div className="nu-explore__controls">
            ...
            <div className="nu-select">
              <select value={faculty} onChange={(e) => setFaculty(e.target.value)} aria-label="Filter by faculty">
                {FACULTIES.map((f) => (
          ...
          <div className="nu-tabs" role="tablist" aria-label="Filter departments by faculty">
            {FILTER_TABS.map((tab) => (
              <button
                key={tab}
                role="tab"
          ...
          <div className="nu-dept-list">
            {visibleDepartments.length === 0 && (
              <p className="nu-dept-list__empty">No departments match your search yet — try a different filter.</p>
            )}
            {visibleDepartments.map((dept) => (
              <Link to="/departments" className="nu-dept-row" key={dept.name}>
                <div className="nu-dept-row__image">
                  <img src={dept.image} alt={dept.name} />
                </div>
                <div className="nu-dept-row__body">
                  <h3>{dept.name}</h3>
                  <p>{dept.description}</p>
                  <span className="nu-badge">{dept.faculty}</span>
                </div>
                <IconArrowRight className="nu-dept-row__arrow" width="18" height="18" />
              </Link>
            ))}
          </div>
        </div>
